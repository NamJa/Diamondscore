...
상태 전환과 라인스코어 확정은 동시에 오고, **투수 요약은 몇 분에서 수십 분 늦게** 온다. 진행 중 `game_result`는 1회초~9회말 라벨을 전부 거친다. 갱신 지연은 1분 간격 캡처로는 측정 불가(서버 캐시 2초).

**종료 전환 재실측 (2026-09-23, 3경기, 1분 간격 캡처)** — 요약 지연이 경기마다 크게 다르다.

| 경기 | `i → e` | `end_summary`·목록 `detail.win_pitcher` 채워짐 | 지연 |
|---|---|---|---|
| 490722 KIA 2:3 두산 (9회말 끝내기) | 21:38:29~21:39:29 | 21:49:12~21:50:13 | 약 10~12분 |
| 490723 롯데 9:5 한화 | 21:49:12~21:50:13 | 21:59:15~22:00:16 | 약 9~11분 |
| 490724 NC 10:3 KT | 21:55:14~21:56:14 | 22:19:22~22:20:22 | **약 23~25분** |

전환 순간 `game_result`가 `w`/`l`, `livecomment.comment_type`이 `fin`, R/H/E 확정이 함께 오는 것은 09-15와 같다. 요약은 `end_summary`와 목록 행 `detail.win_pitcher`가 같은 분에 채워졌다.
...
확정한다(실측상 상태와 함께 온다, §2.4). **투수 요약(`end_summary`)은 종료 뒤 7~25분 사이에 채워지므로**(09-15 7~8분, 09-23 9~25분)
전환 직후 응답엔 `null`이다 — 15초 폴링을 계속 돌리지 말고, **5분마다 요약이 비었는지 확인해 빈 동안만 다시 받다가 30분(6회)에서 멈추고**,
그보다 늦으면 다음에 상세를 열 때 받는다. 지연 조회를 "10분 뒤 1회"로 두면 09-23처럼 요약이 10분을 넘겨 오는 날엔 조회가 요약보다 먼저 나가 투수 행이 끝내 빈다.
`end_summary.pitcher_batter_record.win_pitcher != null`이 요약 확정 신호다. 요약이 비었는지는 **기다린 뒤에** 판정한다 —
...
- [x] `DS-002b` **라이브 종료 관측 (2026-09-15 21:24~21:32)** — `i → e`와 최종 점수·RHEB는 동시, `end_summary`·목록 `detail.win_pitcher`는 **7~8분 뒤** 채워짐(§2.4·§7.3). 2026-09-23 재실측에서는 3경기가 약 10·9·24분 — 지연이 경기마다 흔들린다. 연장 경기의 라이브 표현은 미관측(추가 경기일에 확인)
