...
**디자인 토큰 (다크 우선, 확정 목업 기준)** — Codelabs Step 2가 이 값을 M3 `darkColorScheme`으로 구현한다.

| 역할 | 값 | 역할 | 값 |
|---|---|---|---|
| background | `#0E1116` | 라이브·강조(primary) | `#FF5563` |
| surface(카드) | `#161B22` | 진출권·즐겨찾기(gold) | `#E6B450` |
| surfaceVariant | `#1E242E` | 승(win) | `#3FB980` |
| outline(선) | `#2A313C` | 패(loss) | `#FF6B78` |
| onSurface(본문) | `#E8ECF1` | muted 텍스트 | `#98A2B2` |

- 폰트: 본문은 시스템 한글 산세리프, **점수·기록은 등폭(tabular) 숫자**(라인스코어·순위 정렬). 목업은 `IBM Plex Sans KR/Mono`로 렌더 — 앱에서 동일하게 하려면 폰트를 번들.
- 리그 레드 `#AE0D1D`는 브랜드 기준색, UI 강조는 다크 대비를 위해 밝힌 `#FF5563`을 쓴다.
- 라이트 테마는 같은 역할로 `lightColorScheme`을 하나 더 정의(설정에서 전환). MVP는 다크 우선.
- Material 3 기반, **팀 컬러는 강조에만** 쓰고(§3.4-5의 자체 컬러 자산) 텍스트 대비 WCAG AA 유지.
...
> **주의**: 2026-08-23 APK 분석 환경에서는 같은 엔드포인트가 `403`이었다(웹페이지조차 403). 접근
> 차단은 헤더/인증이 아니라 **호출 출처(IP) 기반**으로 추정된다(§13 A-3). 실기기 접근성은 별도 확인
> 필요 → `DS-001`(§8). 403이면 §1.2 보조 소스로 재설계.
...
- [ ] `DS-001` **실기기/에뮬레이터에서 OkHttp로 `/unique-tournament/11204/seasons` 호출 성공 확인.** 모바일 네트워크와 Wi-Fi 양쪽. 403이면 즉시 중단하고 §1.2 보조 소스로 재설계
...
| 리스크 | 확률 | 영향 | 대응 |
|---|---|---|---|
| **실기기에서 403** | 중 | 치명 | `DS-001`을 최우선 실행. 실패 시 §1.2 보조 소스로 즉시 재설계 |
...
### A-3. 접근 차단의 성격 (`DS-125`)

둘 다 실측이므로 어느 하나를 버리지 않는다.

| 시점 | 환경 | 결과 |
|---|---|---|
| 2026-08-02 | §2.1 검증 네트워크 | ✅ 200, 인증 없음 |
| 2026-08-23 | APK 분석 환경 | ❌ 403 (웹페이지도 403) |

**추정: 헤더/인증이 아니라 호출 출처(IP) 기반 차단.** 인증 없이 200이 나온 사례가 있으므로 "특별한
헤더가 인증 역할을 한다"는 설명은 성립하지 않는다. `DS-001`(실기기 접근)과 함께 네트워크별 상태
코드를 비교해 성격만 규명한다 — 차단 우회는 하지 않는다.
