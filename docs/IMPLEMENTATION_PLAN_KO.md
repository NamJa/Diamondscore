...
**P1을 MVP에서 뺀 근거**: 데이터는 전부 같은 API에 있다(§2.3). 뺀 이유는 커버리지가 아니라 **라이브 스키마
미검증**(§2.4)과 일정이다 — 진행 중 `detail`(볼카운트·주자)이 어떤 시점에 어떻게 채워지는지 관측(`DS-002`)한 뒤에
화면을 만든다. 그 전에는 **UI에 자리도 만들지 않는다** — 추정값으로 채우는 것은 표시 원칙(§1.3)에 위배된다.
...
- **현재 상황(볼·스트라이크·아웃, 주자)**: 상세 응답의 `detail`에 있으나(§3.3) 진행 중 갱신 방식이
  미검증(§2.4)이므로 **P1**. `DS-002` 관측 전에는 자리도 만들지 않는다.
...
실측은 전부 경기 전(오전)·휴식일에 이뤄졌다. 780경기의 `state` 분포는 `a`(예정) 82 · `e`(종료) 630 ·
`c`(취소/노게임) 68이며 **진행 중 값은 관측하지 못했다.**

따라서 다음은 **확인하지 못했다.**

- 진행 중 경기의 `state` 값과 `Schedule_Day` 행의 점수·`inning` 갱신 방식
- 진행 중 `boxscore`의 현재 이닝 표현(`null` / `0` 시점)과 `detail`(볼카운트·주자) 채워지는 시점
- 서버 캐시 2초 대비 실제 갱신 지연

**대응 원칙**: `a`/`e`/`c`는 확정 매핑, **그 외 값인데 점수가 있으면 `LIVE`**, 없으면 `UNKNOWN`으로 둔다(§4.1).
이닝 코드 `bs{N}_{1|2}`는 형식이 확정돼 있어 "N회초/말" 라벨은 파싱으로 만든다(추정이 아님).
...
4. **진행 중 `state` 값이 미검증이다**(§2.4). `a`/`e`/`c` 외의 값인데 점수가 있으면 `LIVE`, 없으면 `UNKNOWN`. `DS-002`에서 확정.
...
fun mapStatus(state: String?, hasScore: Boolean): GameStatus = when (state) {
    ...
    else -> if (hasScore) GameStatus.LIVE else GameStatus.UNKNOWN   // DS-002에서 확정
}
...
- [ ] `DS-002` **게임 데이(18:30 KST 이후) 라이브 관측** — `Schedule_Day/{오늘}`과 `live/schedule/{seq}`를 30초 간격으로 3시간 기록. 확정할 것: 진행 중 `state` 값(§4.1 폴백 제거), 목록 행의 점수·`inning` 갱신 지연, 진행 중 `boxscore` 현재 이닝 표현, `detail`(볼카운트·주자) 채워지는 시점, `end_summary` 확정 시점, `other_game_state` 활용 가능성(§7.1)
...
2. **`DS-002`** — 게임 데이 18:30 KST 경기에서 라이브 응답 3시간 기록. 진행 중 `state` 값이 아직 없어 §4.1이 폴백에 의존한다. **다음 경기일까지 기다려야 하므로 지금 스크립트를 준비**한다(월요일은 휴식일).
