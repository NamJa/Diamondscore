...
- On `i → e` the final score, R/H/E and `livecomment.comment_type == "fin"` arrive together, but `end_summary` (win/loss/save pitchers) and the list row's `detail.win_pitcher` fill **7–25 minutes later** (7–8 min on 2026-09-15; ~10, ~9 and ~24 min for three games on 2026-09-23). Refetch once at the flip, then every 5 min while the summary is still empty, stopping after 6 tries (30 min); never keep the 15s poll running on a `FINAL` game. A single refetch at +10 min misses slow days.
