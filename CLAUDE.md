...
- `state` is `a` scheduled / `e` final / `c` canceled. Canceled no-games keep partial scores and innings — drop them. The in-progress value is unobserved (`DS-002`); anything else with a score maps to `LIVE`, otherwise `UNKNOWN`.
